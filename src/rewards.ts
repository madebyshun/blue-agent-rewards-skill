...
export class RewardsService {
  ...
  private getOrCreateUser(userId: string): UserRecord {
  ...
  private updateStreak(user: UserRecord): { newStreak: number; streakBonus: number } {
    const now = Date.now()
  ...
  async award(userId: string, activity: ActivityType, customPts?: number): Promise<AwardResult> {
  ...
  calculateClaimAmount(user: UserRecord): bigint {
    ...
    if (pts < MIN_CLAIM_POINTS) return 0n
    ...
    // Tier bonus
    let tierBonus = 1.0
    if (pts >= 1000) tierBonus = 1.2
    else if (pts >= 500) tierBonus = 1.1
  ...
  canClaim(user: UserRecord): { ok: boolean; reason?: string; daysLeft?: number } {
